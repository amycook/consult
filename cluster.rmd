...
all7d<- all7c %>% select(mlsto, inv.mlsto, timespan, no.users, Discipline, pc.pro, 
...
cat.knn<- function(df= all7d, predict= 'inv.mlsto', new.cases= c(1000), k=3, result.df= all7d){
        inv.knn = knn(df[,predict], df[new.cases,predict],
                df$pc.pro, k=k)
        indices<- attr(inv.knn,  "nn.index")
        dists<- attr(inv.knn, 'nn.dist') %>% t %>% as.data.frame %>% round(2)
        #checkout the nearest neigbours!
...
b<- cat.knn(df= knn.binary, predict= colnames(knn.binary), new.cases= 60, k=5, result.df= all7d)
...
b<- cat.knn(df= knn.binary, predict= colnames(knn.binary), new.cases= 60, k=20, result.df= all7d)
c<- inv.knn(df= b, predict= 'inv.mlsto', new.cases= 1, k=4) %>% slice(-1)
c
