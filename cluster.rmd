...
all7d<- all7c %>% select(mlsto, inv.mlsto, timespan, no.users, Discipline, pc.pro, 
                         ...
                         Billing.Type, majority.pos, return.pdol, cost.mlsto, hrs.mlsto)
...
#combine waste water and sewerage
all7c[all7c$JD.Second %in% 'sewerage',]$JD.Second<- 'waste water'
...
all7c[all7c$JD.Second %in% c('water cycle management', 'water strategy', 'flood studies'),]$JD.Second<-
        'water management'
...
#Biz.type
table(all7c$group.num, all7c$pc.majpos)
```
...
binary.build(vars= vars.vec, df= all7d) %>% head
```
