...
clust.km<- cascadeKM(clust %>% select(inv.mlsto,suc), 2, 20, iter= 100)
plot(clust.km, sortg = TRUE, grpmts.plot = TRUE)
...
# 11 clusters should be good

# have a look at dendrogram with 11 clusters
...
now want 11 clusters, based on k means optimisation, have a look at average characteristics

```{r, echo=FALSE}

k1<- kmeans(clust %>% select(inv.mlsto, balance.mlsto, return.pdol, suc, hrs.mlsto), 15)
...
# add medians
all7c.med<- ddply(kmeans.plot, .(groups), numcolwise(median)) %>% round(2)
...
clust.summary$bal<- ddply(all7c, .(groups), summarise, bal= round(sum(balance.mlsto), 1)) %>% 
        select(2)

#investigate clusters
all7c %>% filter(groups == 5) %>% select(inv.mlsto, suc, pc.majpos, majority.pos,
...
all7c$groups<- as.integer(all7c$groups)
p<- prcomp(~suc+ inv.mlsto + balance.mlsto + cost.mlsto + return.pdol, data=all7c, centre=T)
# summary(p)
biplot(p, xlabs = as.character(all7c$groups))
#putlers in-house biplot shows centroids
bpCent(p, clsAsgn = all7c$groups, centroids=TRUE, xlabs = as.character(all7c$groups))


#now look at 3D - triplot - using prcomp
#scores stored as 'x' variable
p$x
plot3d(p$x[,1:3], col=as.character(all7c$groups))
text3d(p$x[,1:3], texts = as.character(all7c$groups), color=as.character(all7c$groups), adj=2)
