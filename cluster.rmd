...
qqPlot((all7c$suc+102700) %>% log)

#can't log because of negatives

#density plots - plain variables

ggplot(all7c, aes_string(x='suc')) + geom_histogram(aes(y=..density..))

#density plots - log transformed variables

trial<- (all7c$suc) %>% as.data.frame
ggplot(trial, aes_string(x='.')) + geom_histogram(aes(y=..density..))


### CAN WE TAKE LOG OF ABSOLUTE VALUES AND THEN MAKE NUMBERS THAT WERE PREVIOUSLY NEG NEG AGAIN?

clust$suc<- scale(all7c$suc)
...
Repeat clustering with new suc
