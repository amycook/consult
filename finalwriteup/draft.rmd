...
Testing gradient boosted trees, random forests, naiive bayes, logistic regression and bayesian networks in binary classification showed that boosted trees, logistic regression and random forests performed best (according to AUC). Bayesian Networks are not particularly popular and require discretised variables so it was expected that this algorithm may not perform as well as highly regarded methods such as boosted trees and random forests. Naiive bayes and logistic regression were included as baseline models and it was expected that the more complex models would outperform these. Therefore it was surprising that results from 100 logistic regression models was not statistically lower than boosted trees. Literature??

logistic regression:
- one decision boundary - does not have to be parallel to variable axes
- less prone to overfitting
    - handles many variables better
- weaker predictive power than ensemble trees (An Empirical Comparison of Supervised Learning Algorithms)

ensemble trees:
- can be hundreds of decision boundaries but they have to be parallel to variable axes
- may not be enough data to learn complex decision rules better than a simple model
