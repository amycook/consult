...
The next chapter, Chapter 3 is a thorough review of the literature to date that relates to this thesis. This covers work solving the cost-estimation problem, studies on statistical and machine-learning models in business applications, and a review of effective statistical and machine learning methods. 
...
The research outlined in the method were divided into four stages, each of which was assigned a chapter containing the results and discussion of that portion: 
...
Linear Regression is one of the simplest and most popular statistical prediction methods [@Hastie2009]. An equation for a line is learned, which is defined by a constant and a coefficient multiplied by the value of each variable. The coefficients are chosen to minimise the residual sum of squares (least squared) [@Hastie2009]. The reason for this method's popularity is that it is simple, provides stable predictions, and can be adapted for categorical variables [@Seng2010]. The integration of categorical variables is called Analysis of Variance (ANOVA) Linear Regression, and the algorithm compares group mean variances within the categorical variables. 
...
The method behind Linear Regression can be adapted to the prediction of a binary response variable (1 or 0). A linear equation is fit to the log of the odds of the binary response variable against each of the explanatory variables. The response variable (log odds) in the linear equation can then be translated back to odds by taking the inverse log, i.e. the exponential:

$odds = e^{\ln\  odds}$

Finally the odds is converted to probability by the following equation:
...
The result of this linear fit and logistic translation is a model that predicts the probability (between 0 and 1) that an input case will result in a 0 or 1 (i.e. success or failure) [@Moore1989]. Similar pros and cons exist for Logistic Regression as Linear Regression, and it can be used as a benchmark to compare other binary predictive models for the same reasons. Many business problems have binary response variables, such as yes/no, male/female, buy/do not buy, success/failure, or survival/death [@Moore1989]. 
...
The data contained a rich source of project information and a lengthy process of cleaning the data was required. Then, potentially useful variables were engineered which could be tested alongside original variables. The three initial datasets described:
