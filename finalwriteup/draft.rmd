...
* best models are gradient boosted trees and logistic regression and random forest. why? can hypothesis but not really
* envouraging results - mean sits around 0.775. real question is bottom line analysis
* was this expected? didnt expect logistic regression to perform that well
* imputed data worked almost the same, slightly better for randomforests?
    * wasn't sure what to expect so .. i dunno
* previous research on logistic regression working as well?
* limitations: didn't try all possible variable combinations
* try and improve by blending the three top performing models
    * this will eliminate some of the benefits ofhaving a single model such as 
        * partial dependency plots from boosted tree
