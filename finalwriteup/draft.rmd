...
Several research groups involved in the high profile data science competition, Netflix Prize, developed sophisticated methods of model stacking, otherwise known as ensemble methods or model blending [@Sill2009]. In this research the term 'blending' is used to avoid confusion between ensemble tree methods. The idea behind model blending is to combine predictions from several methods with different theoretical foundations so that the strengths of each method can be combined. Historically, this was called model averaging in statistics and since the 1990's research showed that averaging results from different methods provided better predictive accuracy than any single model [@Madigan1994]. New methods of model blending were developed for the Netflix Prize that applied sophisiticated machine learning techniques to big, messy data. The prize-winning solution was a complex blend of sub-blends that incorporated results of individual models as variables along with original variables. When original variables are used in blending they are called meta-features [@Sill2009]. The multiple layers of blending gave incrememntal improvements in predictive accuracy, but the computational cost of the complex layers did not justify benefits for Netflix in practice and a simplified model was adopted. Therefore, in this project a single layer of blending was trialled.

Five blending methods, ranging from simple to complex were tested on the top performing individual models. These included simple averaging of the individual model results, building a regression model using the individual model results only, feature weighted linear stacking (FWLS), random forests, and boosted trees. The first two models are simply the average or weighted average of the three individual models. The last three methods take advantage of potentially meaningful interactions between an individual model's results and meta-features (the original variables). For example, if the regression predicted profitability better than the other models for Water projects, the model would take advantage of this interaction. Regression predictions would be weighted higher than the other model predictions for Water projects. 

A simplified explanation of FWLS is as a linear regression where meta-features as well as model predictions from individual models are included as variables. Then, each meta-feature is interacted with each column of model results [@Sill2009]. A linear regression is performed to weight each term's contribution to the final prediction. Random forests and boosted trees are also used to blend meta-features with output from several models. Feature interaction is performed passively due to the nature of how trees are built. A split in a node is conditional upon the preceding split based on another variable and so on. 

The five blended models were built on training and testing sets multiple times in a similar fashion to the individual models. A single training set for a blended model must be built from the test results of the individual models. Therefore, five fold cross validation was performed using each individual model to create a full dataset of test results. This was then added to the original dataset, and training and testing data could be partitioned to test the blended models. A maximum of five blended models were created from a complete set of test results, then a new set of test results were created for the next five blended models. This ensured a good mix of training and test data was employed.

To begin, 20 models of each blending method were created. Then, a two-sample power calculation was performed to achieve statistical power of 0.8. The required number of samples (models) were then developed so that the highest performing blended model could be tested against the others for statistical significance. The next step was to assess the blended models' performance on overall profits for the company. This analysis differed from predicitve model accuracy, so several blended models as well as individual models were carried forward for this analysis.
