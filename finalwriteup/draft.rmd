...
comments on your results;
* blended models significantly (?) outperformed individual models
* 

explains what your results mean;

* If the company's profits for one year were $1,000,000, the mean yearly increased profit would be `r (best.mean-100)/100*1000000` by using the best model and rejecting all jobs with a probability higher than `r best.fin %>% filter(method == 'average') %>% .$threshold`. 

interprets your results in a wider context; indicates which results were expected or unexpected;
it was expected that it would show financial benefits and the results look reasonable - not particuarly high or low.
it was also expected that blended model would outperform the single models. Interesting that simple model averaging ended up signficantly (?) outperformiing blended lgoistic regression model in profit curve.

provides explanations for unexpected results.
why did it end up outperforming? no idea.
