...
* as stated in lit review, there is a need for testing more sophisticated ML algorithms such as boosted trees, random forest and bayesian networks on effort estimation. ML algorithms MUST have a way of explaining results. this excludes SVm and neural networks.

* naiive bayes - good baseline
* bayesian network - bad
* random forest 
...
* used AUC for comparison statistic bc... 
* run each method 10 times? statistically significant difference. 10 fold cross validation
* if no statistical difference, run more times -20, 50 75, 100.
* select best wto or three methods and trial model averaging
* improve AUC by model averaging
* choose best method: either a single method or combined

## 3.6 Bottom line translation

* although predictive power is important, for the method to be integrated into the real world, the affect on a business' bottom line must be tested.
* This can be done using a lift chart - a chart that plots the amount of profit the company earns on the y axis vs the probability cut off point on the x axis. For this project, the x-axis represents the point where directors would say 'no' to  a job with a probability for loss greater than that point. There will be an optimal point where saying no to a job above a certain probability level will mean the company earns more profits, becasue they are saying 'no' to jobs that are likely to make a loss. Using the figure of anticipated increase in profit, it will be easier to judge howe much the company would benefit from integrating the algorithm into decision making. 
* the performance of the lift chart will vary depending on the training and test set, so this will be done with 20 fold cross validation, so teh resulting line can be plotted with a confdience interval. A lift chart will be explained in more detail in the results section.


## 3.7 Conclusion/Link to next chapter
