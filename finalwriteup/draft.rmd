...
In ANOVA, linear relationships are tested and normal distributions are assumed for numeric variables. Therefore, where appropriate, numeric variables were norma  


* normalised variables for linear model:
    * log of amount invoiced, total invoiced for client in past projects, % of work done by majority position
    * cube root of timespan
    
* trials with different variable interactions resulted in three interactions that lowered RMSE

* Baseline check: check RMSE from predictions against RMSE if use mean

* ran ANOVA 50 times with core important and largley complete variables
        * amount invoiced
        * Discipline
        * timespan
        * team size
        * pc by professional
        * Business category
* subtract RMSE from model predictions from RMSE using mean return per dollar. If model effective, RMSE is lower for model predictions, and difference is greater than 0. Histogram of difference:
