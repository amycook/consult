...
opts_chunk$set(fig.width=10, fig.depth = 7, dpi = 300, warning = FALSE, message = FALSE, echo = FALSE)
```
...
auc.list <- readRDS("report_data/blend_auc1.rds")
...
The results of the 150 iterations are shown in the plot below. Simple model averaging results in a p-value of `t.test(auc.150[['orig.log']], auc.150[['average']])$p.value %>% format(digits = 2) %>% as.numeric` while the simple logisitc regression blended model had a two-sample t.test p-value of `t.test(auc.150[['orig.log']], auc.150[['simp.log']])$p.value %>% format(digits = 2) %>% as.numeric`.
...
You should point out what the limitations were of your study, and note any questions that remain unanswered. 
* not sure how easy number of users is to predict - could do a study
* could do a study on trust levels
...
Conclusions
could be a viable business move worth integrating

Suggestions

Could collect data for future projects: expected team size, expected time frame.

* trialled ensemble tree methods on cost estimation (in this case project proftability)
* attempted to predict project profitability using a single consulting company's CRM data in the construction industry
* assessed tangible profit outcomes of machine learning models
