...
BT.lin(type= 'aov', data= all7a, add.var = 'Billing.Type')

#loop to extract p values from anova of all additional variables.

#incomplete variabless:
incomp.vars = c('Post.Code', 'Billing.Type', 'Job.Source', 'Job.Detail.Primary', 'Job.Detail.Secondary', 'Job.Type.Primary',
                'Job.Type.Secondary', 'State', 'Type', 'no.employees', 'contact.count', 'JD.Primary', 'JD.Second', 'dist',
                'timespan', 'Num.disc', 'Inv.freq', 'client.invfreq')
plot.pvals = NULL

for(i in seq_along(incomp.vars)){
        formula = paste("return.pdol ~ inv.mlsto + Discipline + client.count + Business +
                              Biz.size + Biz.type + Year + Num.days +
                              no.users + pc.contracttech + client.neginv + client.numinv + client.totinv +
                              pc.director + pc.midtech + pc.midpro + pc.gradpro + pc.seniortech + pc.seniorpro +
                              pc.pro + mean.peeps + hours.perday + num.inv + mean.inv + num.neginv + client.meaninv +
                              code.director + ProjEng.Pos", incomp.vars[i], sep="+")
        
        model= aov(as.formula(formula)              
                      , na.action = na.omit, data= data)
        
        #create columns for results and plotting
        mod.vars = summary(model)[[1]] %>% rownames %>% .[-29]
        mod.vars = sub("\\s+$", "", mod.vars)
        mod.vars = sub(incomp.vars[i], "add.variable", mod.vars)
        p.val = summary(model)[[1]]$'Pr(>F)' %>% .[-29]
        add.variable = rep(incomp.vars[i], 28)
...
#which added variable was sgnificant?
...
* Discipline
* Biz.type
* hours.perday - 0.000455
* code.director - 0.00118
* pc.midtech - 9.11e-07
* no.users = 0.000691
* Num.days = 0.039480
...
load('C:/Users/n9232371/Documents/Consultbusiness/data/objects/all_cfor2.RData')
...
rand.rank$var <- sub("inv.mlsto", "Amount Invoiced for Project", rand.rank$var)
rand.rank$var <- sub("no.users", "Number of employees on Project", rand.rank$var)
rand.rank$var <- sub("client.totinv", "Total Amount Invoiced from the Client - Past Jobs", rand.rank$var)
rand.rank$var <- sub("pc.pro", "% of Hours Completed by a Professional-level Employee", rand.rank$var)
rand.rank$var <- sub("timespan", "Timespan", rand.rank$var)
rand.rank$var <- sub("Business", "Business Category of Client", rand.rank$var)
rand.rank$var <- sub("client.numinv", "Mean Number of Invoices Submitted to Client - past Jobs", rand.rank$var)
rand.rank$var <- sub("client.meaninv", "Mean Invoice Size for Client - Past Jobs", rand.rank$var)
rand.rank$var <- sub("client.count", "Number of Past Jobs with Client", rand.rank$var)
rand.rank$var <- sub("code.director", "Director", rand.rank$var)
rand.rank$var <- sub("Num.disc", "Number of Internal Disciplines Involved", rand.rank$var)
rand.rank$var <- sub("client.neginv", "Number Bad Debt Client Invoices - Past Jobs", rand.rank$var)
rand.rank$var <- sub("Biz.size", "Client Size in Number of Employees", rand.rank$var)
rand.rank$var <- sub("Biz.type", "Broad Client Business Category (Gov, Private etc)", rand.rank$var)
rand.rank = rename(rand.rank, Variable_Description = var, Mean_Rank = m.rank)
rand.rank = rename(rand.rank, Mean_Rank = Rank)

saveRDS(rand.rank, "C:/Users/n9232371/Documents/github/consult/finalwriteup/report_data/RF_var_rank.rds")
saveRDS(rand.rank, "C:/Users/n9232371/Documents/github/consult/finalwriteup/report_data/RF_var_rank_subbed.rds")
rand.rank <- readRDS("C:/Users/n9232371/Documents/github/consult/finalwriteup/report_data/RF_var_rank.rds")
rf.rank <- readRDS("C:/Users/n9232371/Documents/github/consult/finalwriteup/report_data/RF_var_rank_subbed.rds")
