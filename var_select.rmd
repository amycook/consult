...
First need to change code.client and code.contact. Need to assign NA to any code.client or code.contact with less than 3 counts
...
all7c[all7c$Billing.Type %in% c('Estimate','FeeProposal','NoCharge'),]$Billing.Type<- NA
...
all7c$code.client<- ifelse(all7c$V1>2, all7c$code.client, NA)
...
contact.check<- contact.check %>% filter(V1>2)
...
experiment<- all7c
experiment<- experiment %>% select(code.client, Post.Code, Billing.Type, Job.Source, Job.Detail.Secondary, Job.Type.Primary, Type, Project.Value,
                                   no.employees, client.age, contact.count, JD.Primary, JD.Second, dist, code.contact, client.age)
...
vars<- list('Job.Source', 'Job.Detail.Secondary', 'Type', 'Project.Value', 'no.employees','code.client',
            'client.age', 'contact.count', 'JD.Second', 'Billing.Type', c('dist','Post.Code'),
            c('code.contact','contact.count'), c('Job.Type.Primary','JD.Primary'))
cfor.plotsave<- function(extra= vars, name= 'vara'){
        for(i in 1:length(vars)){
                
                core.var= all7c[,names(all7c) %in% c('inv.mlsto', 'timespan', 'no.users', 
                                                     'Discipline', 'pc.pro', 
                        'client.totinv', 'client.numinv','pc.director', 'code.director', 'Num.disc', 
                        'Business', 'Biz.type', 'return.pdol', extra[[i]])]
...
* code.contact - ranks 2nd after timespan/2nd
* code.client (ranks 4th/4th)
* Project.Value (ranks 3rd/4th)
* no.employees - ranks 7th - before code.director  / 3rd last 
* JD.Second - ranks 6th - before code.director and Num.disc / 5th
* Billing.Type - ranks 3rd last but I can't see how this is not important/last
* Job.Type.Primary - ranks 7th - 6 nice broad categories/4th last


Throwing out:
* Job.Source - ranked last / 3rd last
* Job.Detail.Secondary - too patchy I believe / 3rd last
* Type - nice and broad but didn't seem to work - ranked 4th last / 2nd last
* client.age - 3rd last/last
* contact.count - 4th last/3rd last
* dist - 5th last/last
* Post.Code - 6th last or 7th last 
* contact.count - 3rd last/4th last 
* JD.Primary - 3rd last :(/2nd last
...
vars.vec<- c('Job.Source', 'Job.Detail.Secondary', 'Type', 'Project.Value', 'no.employees','code.client',
            ...
            'code.contact','contact.count', 'Job.Type.Primary','JD.Primary')
...
aov.loop<- function(extra=vars.vec){
        for (i in 1:length(extra)){
                ...
                formula = paste("return.pdol ~ inv.mlsto + Discipline + Business+
                        Biz.type + Num.disc +
                        no.users + client.numinv +
                        pc.director + client.totinv + 
                        pc.pro + timespan +
                ...
                assign(paste("summ.",extra[i],sep=""), model, envir=.GlobalEnv)
...
```{r Tukey Ancova visualisation, echo=FALSE}

all.vars<- colnames(all7c %>% select(-mlsto))

auto.BP<- function(colnames=all.vars, yvar = 'return.pdol', df= all7c, name='box'){
        ...
        for (i in 1:length(colnames)){
                ...
                r= ggplot(df[!is.na(df[,colnames[i]]),],
                       aes_string(x=colnames[i], y=yvar, colour = colnames[i])) + 
                        geom_boxplot(alpha = 0, size = .2, outlier.size = .5)+
...
auto.BP(colnames=all.vars, yvar = 'return.pdol', df= all7c, name='box')
